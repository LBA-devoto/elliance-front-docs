...
@Component({
  selector: 'app-creer-personnephysique',
  templateUrl: './creer-personnephysique.component.html',
  styleUrls: ['./creer-personnephysique.component.css'],
})
export class CreerPersonnephysiqueComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.tab.value === 'edition') {
      ...
      let url = this.tab.entite + '/' + this.tab.id; //environment.baseUrl}/${entite}/${id}`
      
      this.httplClientService.get(url).subscribe((res) => {
  ...
  public confrimContact(personnephysique: Personnephysique): boolean {
    let result = false;
    if (personnephysique.nom !== '' && personnephysique.prenom !== '') {
      result = true;
    }
    return result;
  }
